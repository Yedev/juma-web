...
4. **Prompt 结构**：
   ```
   你是 DeepRead 阅读助手。基于以下文章内容回答用户的问题。
   ...
   ```
5. **错误处理**：
   - 未配置 `GEMINI_API_KEY` → 返回 500 `"AI 服务未配置"`
   - Gemini API 请求失败 → 返回 500 `"AI 服务暂不可用"`
   - API 返回空内容 → reply 为 `"抱歉，无法生成回复"`

### 配置要求

```bash
GEMINI_API_KEY="AIzaSy..."   # 在 Google AI Studio 申请
```

### 当前限制

- 无对话历史（每次请求都是独立的单轮对话）
- 不支持流式响应（SSE/Streaming）
- 文章内容限制 8000 字符
